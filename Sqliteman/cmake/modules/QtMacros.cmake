...
macro (QT_EXTRACT_OPTIONS _qt_files _qt_options _qt_target)
  MESSAGE(STATUS "QT_EXTRACT_OPTIONS ${_qt_files} ${_qt_options} ${}_qt_target")
  set(${_qt_files})
  set(${_qt_options})
  set(_QT_DOING_OPTIONS FALSE)
  set(_QT_DOING_TARGET FALSE)
  foreach(_currentArg ${ARGN})
    if ("x${_currentArg}" STREQUAL "xOPTIONS")
      set(_QT_DOING_OPTIONS TRUE)
    elseif ("x${_currentArg}" STREQUAL "xTARGET")
      set(_QT_DOING_TARGET TRUE)
    else ()
      if(_QT_DOING_TARGET)
        set(${_qt_target} "${_currentArg}")
      elseif(_QT_DOING_OPTIONS)
...
macro (QT_MAKE_OUTPUT_FILE infile prefix ext outfile )
  MESSAGE(STATUS "QT_MAKE_OUTPUT_FILE ${infile} ${prefix} ${ext} ${outfile}")
...
macro (QT_GET_MOC_FLAGS _moc_flags)
  MESSAGE(STATUS "QT_GET_MOC_FLAGS ${_moc_flags} ")
  set(${_moc_flags})
...
function (QT_CREATE_MOC_COMMAND infile outfile moc_flags moc_options moc_target)
  MESSAGE(STATUS "QT_CREATE_MOC_COMMAND ${infile} ${outfile} ${moc_flags} ${moc_options} ${moc_target}")
...
macro (QT_GENERATE_MOC infile outfile )
  MESSAGE(STATUS "QT_GENERATE_MOC ${infile} ${outfile}")
  ...
  QT_CREATE_MOC_COMMAND(${abs_infile} ${_outfile} "${moc_flags}" "" "${moc_target}")
...
# QT_WRAP_CPP(outfiles inputfile ... )

macro (QT_WRAP_CPP outfiles )
  MESSAGE(STATUS "QT_WRAP_CPP ${outfiles} ")
  # get include dirs
  QT_GET_MOC_FLAGS(moc_flags)
  QT_EXTRACT_OPTIONS(moc_files moc_options moc_target ${ARGN})

  foreach (it ${moc_files})
    get_filename_component(it ${it} ABSOLUTE)
    QT_MAKE_OUTPUT_FILE(${it} moc_ cxx outfile)
    QT_CREATE_MOC_COMMAND(${it} ${outfile} "${moc_flags}" "${moc_options}" "${moc_target}")
    set_property(SOURCE ${outfile} PROPERTY SKIP_AUTOMOC TRUE)  # don't run automoc on this file
    set_property(SOURCE ${outfile} PROPERTY SKIP_AUTOUIC TRUE)  # don't run autouic on this file
...
macro (QT_WRAP_UI outfiles )
  MESSAGE(STATUS "QT_WRAP_UI ${outfiles} ")
  QT_EXTRACT_OPTIONS(ui_files ui_options ui_target ${ARGN})
...
macro (QT_ADD_RESOURCES outfiles )
  MESSAGE(STATUS "QT_ADD_RESOURCES ${outfiles} ")
  QT_EXTRACT_OPTIONS(rcc_files rcc_options rcc_target ${ARGN})
...
macro(QT_ADD_DBUS_INTERFACE _sources _interface _basename)
  MESSAGE(STATUS "QT_ADD_DBUS_INTERFACE ${_sources} ${_interface} ${_basename} ")
  ...
  add_custom_command(OUTPUT "${_impl}" "${_header}"
      COMMAND ${QTVERSION}::qdbusxml2cpp ${_params} -p ${_basename} ${_infile}
...
macro(QT_ADD_DBUS_INTERFACES _sources)
  MESSAGE(STATUS "QT_ADD_DBUS_INTERFACES ${_sources} ")
  foreach (_current_FILE ${ARGN})
    ...
    QT_ADD_DBUS_INTERFACE(${_sources} ${_infile} ${_basename}interface)
  endforeach ()
endmacro()


macro(QT_GENERATE_DBUS_INTERFACE _header) # _customName OPTIONS -some -options )
  MESSAGE(STATUS "QT_GENERATE_DBUS_INTERFACE ${_header} ")
  QT_EXTRACT_OPTIONS(_customName _qt_dbus_options _qt_dbus_target ${ARGN})
  ...
  add_custom_command(OUTPUT ${_target}
      COMMAND ${QTVERSION}::qdbuscpp2xml ${_qt_dbus_options} ${_in_file} -o ${_target}
      DEPENDS ${_in_file} VERBATIM
  )
endmacro()


macro(QT_ADD_DBUS_ADAPTOR _sources _xml_file _include _parentClass) # _optionalBasename _optionalClassName)
  MESSAGE(STATUS "QT_ADD_DBUS_ADAPTOR ${_sources} ${_xml_file} ${_include} ${_parentClass} ")
  ...
  if(_optionalClassName)
    ...
    add_custom_command(OUTPUT "${_impl}" "${_header}"
       COMMAND ${QTVERSION}::qdbusxml2cpp -m -a ${_basename} -i ${_include} -l ${_parentClass} ${_infile}
       DEPENDS ${_infile} VERBATIM
     )
  endif()
...
macro(QT_AUTOMOC)
  MESSAGE(STATUS "QT_AUTOMOC")
  if(NOT CMAKE_MINIMUM_REQUIRED_VERSION VERSION_LESS 2.8.11)
    message(DEPRECATION "The qt_automoc macro is obsolete. Use the CMAKE_AUTOMOC feature instead.")
  endif()
  QT_GET_MOC_FLAGS(_moc_INCS)
...
macro(QT_CREATE_TRANSLATION _qm_files)
  MESSAGE(STATUS "QT_CREATE_TRANSLATION ${_qm_files} ")
  QT_EXTRACT_OPTIONS(_lupdate_files _lupdate_options _lupdate_target ${ARGN})
  set(_my_sources)
  set(_my_dirs)
  set(_my_tsfiles)
  set(_ts_pro)
...
macro(QT_ADD_TRANSLATION _qm_files)
  MESSAGE(STATUS "QT_ADD_TRANSLATION ${_qm_files} ")
  foreach (_current_FILE ${ARGN})
    get_filename_component(_abs_FILE ${_current_FILE} ABSOLUTE)
...
function(qt_use_modules _target _link_type)
  MESSAGE(STATUS "qt_use_modules ${_target} ${_link_type} ")
  if(NOT CMAKE_MINIMUM_REQUIRED_VERSION VERSION_LESS 2.8.11)
    message(DEPRECATION "The qt_use_modules function is obsolete. Use target_link_libraries with IMPORTED targets instead.")
  endif()
  ...
  foreach(_module ${modules})
    ...
    if (_ucmodule STREQUAL QAXCONTAINER OR _ucmodule STREQUAL QAXSERVER)
      ...
      if ("${_ucmodule}" STREQUAL "MAIN")
        message(FATAL_ERROR "Can not use \"${_module}\" module with qt_use_modules.")
      endif()
    endif()
